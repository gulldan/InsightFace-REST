...
async def dl_image(path):
    ...
    im_data = dict(data=None, traceback=None)
...
def serialize_face(face: Face, return_face_data: bool, return_landmarks: bool = False):
    ...
    if face.embedding_norm:
        _face_dict.update(
            vec=face.normed_embedding.tolist(), norm=float(face.embedding_norm)
        )
    ...
    if face.det_score:
        ...
        if return_landmarks:
            _face_dict.update({"landmarks": face.landmark.astype(int).tolist()})
        ...
        _face_dict.update(
            {
                "facedata": base64.b64encode(
                    cv2.imencode(".jpg", face.facedata)[1].tostring()
                ).decode("utf-8")
            }
        )
...
class Processing:
    ...
    def embed_crops(
        self, images, extract_embedding: bool = True, extract_ga: bool = True
    ):
        ...
        faces = self.model.process_faces(
            iterator,
            ...
            for image in images:
                if image.get("traceback") is not None:
                    ...
                    _face_dict = serialize_face(
                        face=next(faces), return_face_data=False, return_landmarks=False
                    )
    ...
    async def extract(
        ...
        if embed_only:
            _faces_dict = self.embed_crops(
                images, extract_embedding=extract_embedding, extract_ga=extract_ga
            )
            return _faces_dict

        else:
            te0 = time.time()
            output = await self.embed(
                images,
                max_size=max_size,
                return_face_data=return_face_data,
                threshold=threshold,
