...
def parse_model_grpc(model_metadata, model_config):
    ...
    output_names = [
        model_metadata.outputs[i].name for i in range(len(model_metadata.outputs))
    ]
...
class Arcface:
    ...
    def prepare(self, **kwargs):
        ...
        (
            ...
            self.format,
            self.dtype,
        ) = parse_model_grpc(model_metadata, model_config.config)
    ...
    def get_embedding(self, face_img):
        ...
        inputs.append(
            grpcclient.InferInput(
                self.input_name, [face_img.shape[0], self.c, self.h, self.w], "FP32"
            )
        )
        inputs[0].set_data_from_numpy(face_img)

        out = self.triton_client.infer(
            self.model_name, inputs, model_version=self.model_version, outputs=None
        )
...
class Cosface:
    ...
    def prepare(self, **kwargs):
        ...
        self.triton_client.register_cuda_shared_memory(
            self.in_handle_name,
            cudashm.get_raw_handle(self.in_handle),
            0,
            self.input_bytesize,
        )
...
class DetectorInfer:
    ...
    def run(self, input):
        ...
        inputs.append(
            grpcclient.InferInput(self.input_name, [1, self.c, self.h, self.w], "FP32")
        )
        ...
        out = self.triton_client.infer(
            self.model_name, inputs, model_version=self.model_version, outputs=outputs
        )
