    ...
    with open(input_symbol_path, "r") as _input_symbol:
        ...
        for e in fixed_sym["nodes"]:
            ...
            if e["name"] in names:
                e["name"] = f"{e['name']}_1"
            names.append(e["name"])
            if e.get("attrs", {}).get("fix_gamma") == "True" and e["name"].endswith(
                "_gamma"
            ):
...
def arcface_onnx_fixes(onnx_path: str, rewrite: bool = True):
    ...
    for name_rs in reshape_node:
        new_inp = onnx.helper.make_tensor_value_info(
            name=name_rs, elem_type=onnx.TensorProto.INT64, shape=[4]
        )
        new_init = onnx.helper.make_tensor(
            name=name_rs, data_type=onnx.TensorProto.INT64, dims=[4], vals=[1, -1, 1, 1]
        )
...
def convert_insight_model(symbol, params, onnx_path, input_shape=(1, 3, 112, 112)):
    ...
    onnx_mxnet.export_model(
        intermediate_symbol, intermediate_params, [input_shape], np.float32, onnx_path
    )
