...
def _build_engine_onnx(
    input_onnx: Union[str, bytes], force_fp16: bool = False, max_batch_size: int = 1
):
    """
    ...
    with trt.Builder(TRT_LOGGER) as builder, builder.create_network(
        ...
        if force_fp16 is True:
            ...
            if not has_fp16:
                logging.warning(
                    "Builder report no fast FP16 support. Performance drop expected"
                )
...
def convert_onnx(
    ...
    engine = _build_engine_onnx(
        input_onnx=onnx_obj, force_fp16=force_fp16, max_batch_size=max_batch_size
    )
